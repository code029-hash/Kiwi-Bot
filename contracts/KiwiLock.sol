...
import "./IKiwiLock.sol";
import "./IUniswapV2Router02.sol";
import "./IUniswapV2Pair.sol";
import "./IUniswapV2Factory.sol";
import "./FullMath.sol";

contract KiwiLock is IKiwiLock {
    ...
    // ID padding from v1, as there is a lack of a pausing mechanism
    ...
    //         // Calculate the protocol fee as 2% of the locked token amount
    //     uint256 tokenFee = (amount * 2) / 100; // 2% of the token amount

    //     // Deduct the protocol fee from the total amount
    //     uint256 lockedAmount = amount - tokenFee;

    //     // Add the fixed fee of 0.01 Ether
    //     protocolFee = tokenFee + 0.01 ether;

    //     protocolWallet = payable(msg.sender); // Deployer's address
    ...
    //          require(msg.value >= protocolFee, "Insufficient native token balance for protocol fee");
